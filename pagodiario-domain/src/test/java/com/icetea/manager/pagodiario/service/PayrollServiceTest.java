...
public class PayrollServiceTest {
	...
	public void processPeriod_bonus_ok(){
		...
		assertThat("total Liq error", payroll.getTotal(), is(new BigDecimal("36078")));
		assertThat("total Liq amount error", payroll.getTotalAmount(), is(new BigDecimal("36120")));
		assertThat("total Liq discount error", payroll.getTotalDiscount(), is(new BigDecimal(42)));
		// FIXME: la liq supervisor es 0 pq no asocie ningun supervisor ...
		assertThat("total Liq total supervisor error", payroll.getTotalSupervisor(), is(new BigDecimal("18039")));
		
		List<SupervisorPayrollItem> supervisorPayrollItemList = payroll.getSupervisorPayrollItemList();
		
		assertThat("error supervisorPayrollItemList.size() != 1", supervisorPayrollItemList.size(), is(1));
		SupervisorPayrollItem supervisorPayrollItem = supervisorPayrollItemList.get(0);
		assertThat("error supervisorPayrollItem totalAmount", supervisorPayrollItem.getTotalAmount(), is(new BigDecimal("18039")));
		...
		PayrollItem payrollItem = payroll.getPayrollItemList().get(0);
		assertThat("total Liq Item collect error no 600", payrollItem.getSubtotalCollect(), is(new BigDecimal("36120")));
		assertThat("total Liq Item discount error no 100", payrollItem.getSubtotalDiscount(), is(new BigDecimal(42)));
		assertThat("total Liq Item total error no 500", payrollItem.getTotalAmount(), is(new BigDecimal("36078")));
		
		assertThat("error payroll items != 2", payrollItem.getItems().size(), is(1));
		
		BonusConciliationItem bonusItem = payrollItem.getBonusItem();
		
		assertThat("bonusItem error", bonusItem.getCollectAmount(), is(new BigDecimal("6020")));
		assertThat("bonusItem error", bonusItem.getDescription(), is("PREMIO 20% (productos periodo(301)/Dias Hábiles)"));
	}
	...
	public void processPeriod_bonus_some_traders_same_supervisor_get_bonus_ok(){
		...
		assertThat("total Liq error", payroll.getTotal(), is(new BigDecimal("39874")));
		assertThat("total Liq amount error", payroll.getTotalAmount(), is(new BigDecimal("40000")));
		assertThat("total Liq discount error", payroll.getTotalDiscount(), is(new BigDecimal(126)));
		// FIXME: la liq supervisor es 0 pq no asocie ningun supervisor ...
		assertThat("total Liq total supervisor error", payroll.getTotalSupervisor(), is(new BigDecimal("14958")));
		
		List<SupervisorPayrollItem> supervisorPayrollItemList = payroll.getSupervisorPayrollItemList();
		
		assertThat("error supervisorPayrollItemList.size() != 1", supervisorPayrollItemList.size(), is(1));
		SupervisorPayrollItem supervisorPayrollItem = supervisorPayrollItemList.get(0);
		assertThat("error supervisorPayrollItem totalAmount", supervisorPayrollItem.getTotalAmount(), is(new BigDecimal("14958")));
		...
		PayrollItem payrollItem = payroll.getPayrollItemList().get(0);
		assertThat("total Liq Item collect error no 600", payrollItem.getSubtotalCollect(), is(new BigDecimal("12000")));
		assertThat("total Liq Item discount error no 100", payrollItem.getSubtotalDiscount(), is(new BigDecimal(42)));
		assertThat("total Liq Item total error no 500", payrollItem.getTotalAmount(), is(new BigDecimal("11958")));
		
		assertThat("error payroll items != 2", payrollItem.getItems().size(), is(1));
		
		BonusConciliationItem bonusItem = payrollItem.getBonusItem();
		
		assertThat("bonusItem error", bonusItem.getCollectAmount(), is(new BigDecimal("2000")));
		assertThat("bonusItem error", bonusItem.getDescription(), is("PREMIO 20% (productos periodo(250)/Dias Hábiles)"));
	}
